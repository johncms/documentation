...
* **fields** - содержит массив полей, которые будут доступны в шаблоне, а так же будут использоваться для отправки:
  ...
  * **reply\_to** - Адрес, на который уйдёт ответ, если отвечать нужно не на адрес сайта. Например, в уведомлении из формы обратной связи это адрес написавшего посетителя. (**не обязательно**)
  * **reply\_to\_name** - Имя для адреса ответа. (**не обязательно**)
  * **cc**, **bcc** - Копия и скрытая копия. Принимают как один адрес строкой, так и массив адресов. (**не обязательно**)

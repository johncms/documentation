...
`titleHtml` уже очищен [санитайзером](html-sanitizer.md) по политике `HtmlPolicy::Inline` — из разметки остаются только инлайновые теги (`a`, `b`, `strong`, `i`, `em`, `u`, `br`), блочные элементы и всё небезопасное вырезаются. Поэтому в шаблоне он печатается как разметка, без экранирования.

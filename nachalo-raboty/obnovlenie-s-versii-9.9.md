...
подключает их через `{% raw %}{{ vite('public') }}{% endraw %}` — какой именно файл подставить,
система берёт из того же раздела `entries`.
Картинки, шрифты и прочая статика по-прежнему лежат в `public/themes/<ваш шаблон>/assets/`, и
функция `asset()` в шаблонах работает как прежде.
...
* **Подключение макета.** Вместо `$this->layout('system::layout/default')` страница пишет
  `{% raw %}{% extends '@theme/layouts/default.twig' %}{% endraw %}`, а её содержимое кладётся в
...
* **Вывод данных.** Вместо `<?= $this->e($value) ?>` — `{% raw %}{{ value }}{% endraw %}`.

...
писем, слаги в библиотеке и загрузках. Уже существующее не трогается, повторный запуск безопасен.
Прежние команды `auth:upgrade-schema`, `mail:upgrade-schema`, `library:generate-slugs` и
`downloads:generate-slugs` удалены — всё это делает `migrate`.
...
Таблицы ролей и сессий создаёт `migrate` — отдельная команда для этого больше не нужна. После
него выполните перенос данных **в этом порядке**:

```bash
php system/bin/console auth:migrate-rights             # должности → роли
php system/bin/console auth:apply-default-permissions  # права встроенных ролей
php system/bin/console auth:migrate-module-access      # настройки mod_* → права ролей
php system/bin/console auth:drop-legacy-columns        # удаление старых колонок
```

Последняя команда удаляет данные и отказывается работать, если `auth:migrate-rights` ещё не
выполнялась: сначала должности должны стать ролями. До неё сайт работает и на старых колонках,
так что между шагами можно остановиться и проверить, что всё на месте.

Эти четыре шага остались отдельными командами намеренно: они переносят данные по правилам, в
которых у владельца сайта есть выбор, а последняя ещё и удаляет колонки безвозвратно. Такое
решение принимает человек, а не автоматическое обновление.

...
Сборка перешла с webpack на Vite. Точки входа перечислены в `vite.config.js` в корне сайта,
собранные файлы попадают в `public/build/`, а шаблон подключает их через `{% raw %}{{ vite('public') }}{% endraw %}`.
Какой файл подставить, система берёт из манифеста темы — раздела `entries` в `theme.php`.

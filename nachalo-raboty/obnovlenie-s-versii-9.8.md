...
### Модуль «Контакты»: создание таблицы

В версии 9.9 добавлен новый модуль **«Контакты»** (`contacts`) — публичная страница с контактной информацией и формой обратной связи. При обновлении существующей установки его таблица не создаётся автоматически. Поэтому после обновления нужно один раз выполнить скрипт из папки `install`:

```bash
cd install
php install_contacts.php
```

Скрипт создаёт таблицу `contact_messages`, в которой хранятся полученные обращения.

Особенности:

* Скрипт **безопасен при повторном запуске**: если таблица `contact_messages` уже существует, он ничего не делает и сообщает, что модуль установлен.
* Модуль должен быть перечислен в `installed_modules` в файле `config/autoload/modules.global.php`. В штатной поставке он там уже есть; если конфигурационный файл правился вручную, добавьте `'contacts'` в список, иначе скрипт не найдёт класс установщика модуля.
* Так как добавлен новый PSR-4 неймспейс `Johncms\Modules\Contacts\`, после выкладки кода нужно обновить автозагрузчик Composer: `composer dump-autoload`.

После установки настройки контактов доступны в админ-панели по адресу `/admin/contacts`, полученные обращения — по `/admin/contacts/messages`, а публичная страница — по `/contacts/`.

Что нужно сделать после установки:

* Заполнить контактные данные в админке (`/admin/contacts`): e-mail, телефон, адрес, часы работы, ссылки на соцсети. Текстовые поля мультиязычные: если значение для текущего языка не заполнено, показывается значение для языка сайта по умолчанию, а если и его нет — английское.
* Включить форму обратной связи флажком «Включить форму обратной связи» — по умолчанию она выключена, и на странице выводится только контактная информация.
* При необходимости указать «E-mail для уведомлений». Если поле пустое, уведомления о новых обращениях уходят на e-mail сайта из системных настроек.
* Уведомления отправляются через **очередь писем**, поэтому у сайта должен быть настроен планировщик (cron), который её разбирает. Без него обращения продолжат сохраняться в базе, но письма отправляться не будут.
* Если нужно согласие на обработку персональных данных на форме, добавьте в модуле «Согласия» (`/admin/consents`) активное согласие с контекстом `contacts` — его чекбокс появится на форме, а факт принятия попадёт в журнал.

...
* **Через пулреквест на GitHub** — отредактируйте нужный `.po`-файл (`system/locale/<язык>.po` или `modules/<модуль>/locale/<язык>.po`) и отправьте пулреквест. Подробнее — на странице [Перевод JohnCMS на другие языки](perevod-johncms-na-drugie-yazyki.md).

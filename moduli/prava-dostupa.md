...
```bash
php system/bin/console auth:apply-default-permissions --force
```
...
Значения старых настроек доступа переносятся в права ролей одной командой:

```bash
php system/bin/console auth:migrate-module-access
```

Она читает `mod_forum`, `mod_guest`, `mod_lib`, `mod_down`, `mod_reg` и `active` и выдаёт (или
снимает) соответствующие права ролям `guest` и `user`. Единственная команда в системе, которая
права **снимает**: «форум только авторизованным» — это роль `guest` без `forum.view`.

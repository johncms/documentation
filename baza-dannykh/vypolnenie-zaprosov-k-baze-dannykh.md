...
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/5NJeWEeBonlrBhBrVHEz/baza-dannykh/vypolnenie-zaprosov-k-baze-dannykh
---
...
Метод **paginate** конструктора запросов в JohnCMS не используется: постраничную навигацию
строит сервис `PaginationFactory`, а запрос получает явные **limit** и **offset**.

```php
// В контроллере
$total = $connection->table('users')->count();
$pagination = $this->paginationFactory->create($total, 5);
...
return new ViewResponse('@partners/public/index.twig', [
    'users'      => $users,
    'pagination' => $pagination->render(),
]);
```

Метод `render()` возвращает готовую разметку навигации, а адреса страниц строятся из текущего
...
**themes/default/templates/components/pagination.twig**
